...
unsigned int MINI_WIDTH = 1920/2;
unsigned int MINI_HEIGHT = 1080/2;
Camera camera;
...
unsigned int alt_fbo;
unsigned int alt_fbo_tex;
unsigned int alt_rbo;

void setupAltFramebuffer() {
    unsigned int fbo;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, MINI_WIDTH, MINI_HEIGHT, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
    ...
    // bind a renderbuffer for the stencil/depth info
    unsigned int rbo;
    glGenRenderbuffers(1, &rbo);
    glBindRenderbuffer(GL_RENDERBUFFER, rbo);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, MINI_WIDTH, MINI_HEIGHT);
...
void renderAltFramebuffer(std::function<void(void)> renderScene) {
    glViewport(0, 0, MINI_WIDTH, MINI_HEIGHT); {
...
int main()
{
    ...
    setupAltFramebuffer();
    // create a quad
    auto quadW = (float)MINI_WIDTH / SCR_WIDTH;
    auto quadH = (float)MINI_HEIGHT / SCR_HEIGHT;
    auto quad = std::make_shared<ControlledMesh>(ControlledMesh::CreateQuad(quadW, quadH));
    quad->SetPosition(glm::vec3{ -1 + quadW/2.0, -1 + quadH/2.0, -1 });
    quad->SetScale(1.5f);
    quad->AddTexture(Texture(alt_fbo_tex, "texture_diffuse", ""));
    Shader quadShader(SHADERS_DIR "quad.vert", SHADERS_DIR "quad.frag");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // alternate framebuffer
        renderAltFramebuffer(renderScene);
        quadShader.use();
    ...
    // ------------------------------------------------------------------------
    glDeleteFramebuffers(1, &alt_fbo);
